...
module Parse = Bool_lang.Parse
module Ast = Bool_lang.Ast
module To_cfg = Bool_lang.To_cfg
...
let run ?(bench = false) ?(meth = "K") filename =
  ...
  | `Yes ->
    In_channel.with_file filename ~f:(fun ch ->
        ...
        let module V = struct
          let vars = List.map prog.gvars ~f:(fun gvar -> gvar.txt)

          type formula = Ast.expression

          let pp_formula = Ast.pp_expression

          let destruct_formula e =
            match e.Ast.exp_desc with
            | E_true -> `Tru
            | E_false -> `Fls
            | E_var x -> `Var x.txt
            | E_and (e1, e2) -> `And (e1, e2)
            | E_or (e1, e2) -> `Or (e1, e2)
            | E_not e0 -> `Not e0
          ;;
        end
        ...
        if not bench then List.iter (List.take sol 10) ~f:(fun (x, mat) -> Format.printf "%s = %a" x D.pp mat);
...
let main () =
  ...
  let driver =
    Command.group
      ~summary:"Bayesian-inference Analysis"
      [ ( "analyze"
        , Command.basic
            ~summary:"analyze"
            Command.Let_syntax.(
              ...
              fun () -> run filename ~meth ~bench |> report_result) )
      ]
  in
  Command.run driver
;;
